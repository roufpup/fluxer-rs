use serde::Deserialize;
use serde_json::Value;

use crate::serde::types::user::RtcRegion;

/// Data for a channel, received from a `CHANNEL_UPDATE` dispatch event. <br>
#[derive(Deserialize, Debug)]
pub struct ChannelData {
    pub guild_id: String,
    pub id: String,
    pub name: String,
    pub permission_overwrites: Vec<Value>,
    pub position: i64,
    #[serde(rename = "type")]
    pub channel_type: i64,
    pub last_message_id: Option<String>,
    pub last_pin_timestamp: Option<Value>,
    pub nsfw: Option<bool>,
    pub parent_id: Option<String>,
    pub rate_limit_per_user: Option<i64>,
    pub topic: Option<Value>,
    pub bitrate: Option<i64>,
    pub rtc_region: Option<RtcRegion>,
    pub user_limit: Option<i64>,
}

/// Data for a guild member, received from a `GUILD_MEMBER_UPDATE` dispatch event. <br>
#[derive(Deserialize, Debug)]
pub struct MemberData {
    pub accent_color: Option<Value>,
    pub avatar: Option<Value>,
    pub banner: Option<Value>,
    pub communication_disabled_until: Option<Value>,
    pub deaf: bool,
    pub guild_id: Option<String>,
    pub joined_at: String,
    pub mute: bool,
    pub nick: Option<Value>,
    pub roles: Option<Vec<String>>,
    pub user: Option<UserData>,
}

/// Data for a user, used in [`MemberData`]
#[derive(Deserialize, Debug)]
pub struct UserData {
    pub avatar: Value,
    pub avatar_color: Value,
    pub bot: Option<bool>,
    pub discriminator: String,
    pub flags: i64,
    pub global_name: Value,
    pub id: String,
    pub username: String,
}

/// Data for a guild's properties, received from a `GUILD_UPDATE` dispatch event.
#[derive(Deserialize, Debug)]
pub struct GuildPropertiesData {
    pub afk_channel_id: Value,
    pub afk_timeout: i64,
    pub banner: Value,
    pub banner_height: Value,
    pub banner_width: Value,
    pub default_message_notifications: i64,
    pub disabled_operations: i64,
    pub embed_splash: Value,
    pub embed_splash_height: Value,
    pub embed_splash_width: Value,
    pub explicit_content_filter: i64,
    pub features: Vec<String>,
    pub icon: Value,
    pub id: String,
    pub message_history_cutoff: Value,
    pub mfa_level: i64,
    pub name: String,
    pub nsfw_level: i64,
    pub owner_id: String,
    pub rules_channel_id: Value,
    pub splash: Value,
    pub splash_card_alignment: i64,
    pub splash_height: Value,
    pub splash_width: Value,
    pub system_channel_flags: i64,
    pub system_channel_id: Option<String>,
    pub vanity_url_code: Value,
    pub verification_level: i64,
}

/// Data for a role, returned as part of other data types
#[derive(Deserialize, Debug)]
pub struct RoleData {
    pub id: String,
    pub name: String,
    pub color: i64,
    pub position: i64,
    pub hoist_position: Option<i64>,
    pub permissions: String,
    pub hoist: bool,
    pub mentionable: bool,
}

/// Data for a message's author, used in [`MessageData`](crates::serde::types::message::MessageData)
#[derive(Deserialize, Debug)]
pub struct AuthorData {
    pub avatar: Option<String>,
    pub avatar_color: Option<i64>,
    pub discriminator: String,
    pub flags: i64,
    pub global_name: Option<String>,
    pub id: String,
    pub username: String,
}

/// Data for an emoji returned as part of other data types
#[derive(Deserialize, Debug)]
pub struct EmojiData {
    pub name: String,
    pub animated: Option<bool>,
    pub id: Option<String>,
}
